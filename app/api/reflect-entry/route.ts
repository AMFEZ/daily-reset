...
export async function POST(request: Request) {
  try {
    ...
    const body =
      (await request.json()) as ReflectEntryBody;
    ...
    const {
      ...
      .select(
        "id, user_id, entry_type, title, content, mood, energy, tags"
      )
    ...
    if (
      ...
      !supportedEntryTypes.has(entry.entry_type)
    ) {
      return NextResponse.json(
        {
          error:
            "This entry type is not supported by reflect-entry.",
        },
        {
          status: 400,
        }
    ...
    if (entryContent.length < 2) {
      return NextResponse.json(
        {
          error:
            "Journal entry has no content to reflect on.",
        },
        {
          status: 400,
        }
...
`.trim();
    ...
    if (!outputText) {
      return NextResponse.json(
        {
          error:
            "Reflection returned empty output.",
        },
        {
          status: 500,
        }
